...
void Battery_init(void)
{
	...
	battery_timer = xTimerCreate((const char*		)"BatteryTimerTimer",
								(TickType_t			)100000*60*5,
								(UBaseType_t		)pdTRUE,
								(void*				)1,
								(TimerCallbackFunction_t)Battery_timerHandler); //周期定时器，周期5min(50个时钟节拍)，周期模式
...
float Battery_calVoltage(u16 adcVal)
{
  ...
  return ((float)adcVal*(3.3/4096));       //胡广豪 :这里可以优化，把除法改成移位
