...
{ 
	char font_flag = 0;			//字库标志位 0000 0000 		64*64	32*32	24*24	16*16
	char font_path[100];	//保存字库地址，用于判断字库是否存在
	...
	{ 
		...
		{ 
			...
			{ 
				...
				/*判断是否存在字库文件*/
				sprintf(font_path,"%s/%s",path,fn);
				if(strcmp(FONT_16,font_path) == 0)	font_flag += 1<<0;
				if(strcmp(FONT_24,font_path) == 0)	font_flag += 1<<1;
				if(strcmp(FONT_32,font_path) == 0)	font_flag += 1<<2;
				if(strcmp(FONT_64,font_path) == 0)	font_flag += 1<<3;
			}//else
		} //for
	} 
	return font_flag; 
