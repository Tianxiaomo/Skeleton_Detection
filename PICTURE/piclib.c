...
{
	if((len==0)||(x0>oleddev.width)||(y0>oleddev.height))return;
	Fill_Block(x0,x0+len-1,y0,y0,color);	
}
//填充颜色
//x,y:起始坐标
//width，height：宽度和高度。
...
void OLED_Fill(u16 x,u16 y,u16 x1,u16 y1,u32 color){
	Fill_Block(x,x1,y,y1,color);
}
//////////////////////////////////////////////////////////////////////////
...
//	pic_phy.read_point=LCD_ReadPoint;  		//读点函数实现,仅BMP需要
	pic_phy.draw_point=draw_point;	//画点函数实现
	pic_phy.fill	=	OLED_Fill;					//填充函数实现,仅GIF需要
	pic_phy.draw_hline=piclib_draw_hline;  	//画线函数实现,仅GIF需要
	pic_phy.fillcolor=OLED_Fill_Color;  	//颜色填充函数实现,仅TJPGD需要 

	picinfo.lcdwidth  = oleddev.width;	//得到LCD的宽度像素
	picinfo.lcdheight = oleddev.height;//得到LCD的高度像素
